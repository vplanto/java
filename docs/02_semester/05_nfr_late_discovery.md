...
- UI та Client-Side: Зміна NFR на високе навантаження змушує змінити поведінку клієнта (відмова від polling на користь WebSocket/SSE). Крім того, кардинально змінюються макети інтерфейсу користувача (UI): доведеться додати візуальні індикатори "живого" стану, скелетони (skeleton screens) обробки ставки, та складнішу обробку втрати з'єднання без блокування екрану.
...
### 4.2 Техніка: NFR Workshop у Життєвому Циклі (SDLC)

Замість того щоб просити NFR у документ — проводьте 2-годинний NFR Workshop зі стейкхолдерами.

Згідно з еталонами життєвого циклу інформаційних систем (SDLC), NFR Workshop має проводитися на етапі **Requirements Analysis (Аналіз вимог)** або, щонайпізніше, на початку **System Architecture / Design (Проєктування системи)** — обов'язково *до* старту етапу Implementation (Розробки коду). У гнучких методологіях (Agile/Scrum) цей процес зазвичай вбудовують в етап **Product Discovery** (Фаза відкриття або Inception phase), формуючи базове архітектурне бачення (Architecture Vision), або проводять як **Continuous Backlog Refinement** перед стартом критичних епіків.
...
### Architectural Decision Records (ADR) та Об'єктно-Орієнтоване Проєктування

ADR — короткий документ, який фіксує: яке рішення прийнято, чому, і від яких альтернатив відмовились.

У контексті об'єктно-орієнтованого проєктування (OOD) ПЗ, саме NFR безпосередньо диктують вибір компонентів, патернів та організації об'єктів. Наприклад: якщо NFR вимагає Flexibility (гнучкості), архітектор застосує патерни *Strategy* або *Dependency Injection*. Якщо ключовий NFR — Performance та економія пам'яті, структура класів базуватиметься на патернах *Flyweight* або пулах об'єктів (*Object Pool*). ADR ідеально підходить для того, щоб зафіксувати, який саме NFR зумовив вибір конкретної компонентної моделі чи ООП-патерну.
...
- Cassandra: overkill для 500 RPS, складна операційно.
