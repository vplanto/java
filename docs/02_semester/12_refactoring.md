...
**Золоте правило:** до і після *самого кроку* рефакторингу всі наявні тести мають проходити. Під час рефакторингу поведінка не змінюється, тому нові тести не додаються. Але якщо тестів бракує, їх необхідно написати *до* початку рефакторингу як окремий підготовчий крок.
...
| Недосвідченість команди | Написання «коду-спагеті» або сліпе копіювання зі StackOverflow |
...
- `Switch Statements` — дублювання довгого `switch` (або `if-else`) залежно від типу об'єкта. Додавання нового типу вимагає змін у багатьох місцях (порушення Open-Closed Principle). Замінюється поліморфізмом (наприклад, патерном Strategy).
...
```
Перед додаванням нової фічі     → спочатку приберіть безлад, потім додавайте
Коли код важко зрозуміти        → якщо читаєш і думаєш «що це?» — рефакторити
...
Немає тестів взагалі       → сліпий рефакторинг небезпечний, спочатку напишіть тести
...
### Філософія розробки: «Make it work → Make it beautiful → Make it perfect»

Написати ідеальний код з першої спроби практично неможливо. Спроба зробити все одразу часто призводить до «паралічу аналізу». Тому професіонали використовують трифазний підхід:

> **Make it work → Make it beautiful → Make it perfect**

1. **Make it work (Зроби, щоб працювало):** Спочатку напишіть найпростіше рішення, яке вирішує бізнес-проблему. Не бійтеся «брудного» коду, дублювання чи тимчасових змінних на цьому етапі. Головне — код має працювати та проходити тести.
2. **Make it beautiful (Зроби красиво):** Коли код уже працює і ви впевнені у логіці, наведіть лад. Це і є фаза рефакторингу. Покращуйте іменування, виділяйте методи, структуруйте класи. Тепер ви думаєте лише про чистоту коду, не відволікаючись на пошук рішень для бізнес-задач.
3. **Make it perfect (Доведи до ідеалу):** Лише якщо це дійсно потрібно (наприклад, для високонавантажених частин системи), оптимізуйте продуктивність чи використання ресурсів. Передчасна оптимізація — ворог гнучкості та підтримки.
...
public void logOrder(Order order) {
    ...
    for (Item item : order.getItems()) {
        ...
        log.info("  {} x{}", item.getName(), item.getQuantity());
    }
...
public void logOrder(Order order) {
    logHeader(order);
    logCustomer(order.getCustomer());
    logItems(order.getItems());
    logTotal(calculateTotal(order));
}

private void logHeader(Order order) {
    log.info("=== Order #{} ===", order.getId());
}
