...
> Тепер ліміт книг можна змінити через змінну середовища при запуску (наприклад, `export APP_LIBRARY_MAX_BOOKS=200` або в Docker: `-e APP_LIBRARY_MAX_BOOKS=200`) або через аргумент командного рядка (`--app.library.max-books=200`), не чіпаючи код та не перекомпілюючи програму.
