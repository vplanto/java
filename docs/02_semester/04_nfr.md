...
Вимага звучить так: «P99 latency < 500мс при навантаженні 1,000 RPS на ендпоінт /checkout».

</details>

### Види тестування продуктивності (Performance Testing)

Щоб виміряти та підтвердити зазначені метрики (RPS, Latency, P99), QA-інженери використовують спеціалізовані види тестування:

- **Load Testing (Навантажувальне тестування):** Перевірка поведінки системи під очікуваним (номінальним) рівнем навантаження. Допомагає переконатися, що система відповідає цільовим показникам.
- **Stress Testing (Стрес-тестування):** Поступове збільшення навантаження понад норму до моменту відмови системи. Мета — знайти "вузьке місце" (bottleneck) та перевірити здатність системи до відновлення.
- **Spike Testing (Стрес-стрибки):** Перевірка реакції системи на **миттєвий** екстремальний стрибок навантаження (наприклад, той самий сценарій Black Friday, коли 50,000 людей заходять одночасно за 1 хвилину). На відміну від Stress Testing, тут навантаження не росте плавно, а вдаряє "лавиною".
- **Scalability Testing (Тестування масштабованості):** Перевірка поведінки системи при збільшенні ресурсів (горизонтальне/вертикальне масштабування).
- **Stability / Soak Testing (Тестування стабільності):** Запуск системи під середнім навантаженням протягом тривалого часу (24-72 години) для виявлення витоків пам'яті (memory leaks) та деградації продуктивності.
...
> 🛡️ **Важливо:** Security як нефункціональна вимога є **невід'ємною частиною стратегії управління якістю ПЗ (QA Strategy)**. Безпеку неможливо "прикрутити" в кінці проєкту. Вимоги до безпеки повинні автоматично перевірятися в CI/CD як Quality Gates (наприклад, через статичний аналіз вразливостей або автоматизований DAST).
...
> ⚙️ **Важливо:** Maintainability також є **невід'ємною частиною стратегії управління якістю ПЗ (QA Strategy)**. Такі метрики не перевіряються "ручними тестувальниками", а вимагають вбудованих автоматизованих перевірок (наприклад, SonarQube для вимірювання технічного боргу). Без постійного фокусу QA на цих метриках підтримуваність системи невідворотно деградує.
...
NFR не живуть у повітрі — вони фіксуються в документах, насамперед у **Технічному Завданні (ТЗ) або Software Requirements Specification (SRS)**.

### Формалізація в ТЗ (SRS)

У професійному ТЗ нефункціональні вимоги описуються окремо (часто спираючись на моделі ISO/IEC 25010):
1. **Метрики як Acceptance Criteria:** Кожна вимога описується так, щоб QA інженер міг її перевірити. Замість "швидко масштабується" — "додає новий вузол за <2 хв при навантаженні CPU >80%".
2. **Traceability Matrix:** NFR отримує ідентифікатор (напр., `NFR-PERF-01`), який пов'язують з відповідними тест-кейсами продуктивності та архітектурними рішеннями.
3. **Специфікація SLA:** Чітко визначаються цифри доступності, maintenance вікна та допустимий рівень деградації у стресових умовах.
