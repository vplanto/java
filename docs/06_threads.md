...
**Питання 1: Чому метод `run()` не є еквівалентом методу `start()` при роботі з потоками в Java?**

<details markdown="1">
<summary>Еталонна відповідь</summary>

Метод `start()` ініціює новий потік і в ньому виконує метод `run()`. Якщо виклик методу `run()` здійснюється безпосередньо, він виконується в тому ж потоці, де був викликаний, і новий потік не створюється. Це суперечить принципу паралельної обробки.

</details>

**Питання 2: Поясніть концепцію та небезпеку "взаємного блокування" (Deadlock)?**

<details markdown="1">
<summary>Еталонна відповідь</summary>

Deadlock — це ситуація, коли два або більше потоки назавжди блокують один одного, оскільки кожен з них чекає на ресурс, який захоплений іншим потоком. Наприклад, Потік А тримає Ресурс 1 і чекає на Ресурс 2, а Потік Б тримає Ресурс 2 і чекає на Ресурс 1. Програма при цьому "зависає" без можливості виходу.

</details>

**Питання 3: В чому різниця між `synchronized` методом та використанням `ReentrantLock`?**

<details markdown="1">
<summary>Еталонна відповідь</summary>

`synchronized` — це вбудований у мову механізм, який автоматично захоплює та звільняє замок (монітор) об'єкта. Він простий, але недостатньо гнучкий.

`ReentrantLock` — це явний замок з пакету `java.util.concurrent.locks`. Він дозволяє вручну керувати методами `lock()` та `unlock()`, що дає більше контролю (можливість перевірити чи замок вільний через `tryLock()`, встановлювати черговість `fairness` тощо).

</details>

**Питання 4: Які існують основні стани життєвого циклу потоку в Java (Thread Lifecycle)?**

<details markdown="1">
<summary>Еталонна відповідь</summary>

Основні стани включають:
1. **NEW**: Потік створено, але не запущено.
2. **RUNNABLE**: Потік готовий до виконання або вже виконується.
3. **BLOCKED**: Потік чекає на звільнення замка (synchronized).
4. **WAITING/TIMED_WAITING**: Потік очікує на дію іншого потоку (з або без таймауту).
5. **TERMINATED**: Потік завершив своє виконання.

</details>

---

**[Лекція 5: SOLID](05_solid.md)** | **[Лекція 7: API Design ➡️](07_api_design.md)**
